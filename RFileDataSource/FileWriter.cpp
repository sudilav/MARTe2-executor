...
ErrorManagement::ErrorType FileWriter::OpenFile(StreamString filenameIn) {
    ...
    if (!fatalFileError) {
        ...
        if (fileFormat == FILE_FORMAT_CSV) {
            ...
            uint32 SIGNAL_NAME_MAX_SIZE = 32u;
            for (n = 0u; (n < nOfSignals) && (!fatalFileError); n++) {
                StreamString signalName;
                fatalFileError = !GetSignalName(n, signalName);
                if (signalName.Size() > SIGNAL_NAME_MAX_SIZE) {
                    SIGNAL_NAME_MAX_SIZE = signalName.Size();
                }
            }
            fatalFileError = !outputFile.Write(reinterpret_cast<const char8*>(&SIGNAL_NAME_MAX_SIZE), writeSize);
            for (n = 0u; (n < nOfSignals) && (!fatalFileError); n++) {
